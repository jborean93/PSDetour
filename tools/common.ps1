...
function Assert-Detours {
    ...
    param(
        ...
        if (Test-Path -LiteralPath $targetFolder\Detours) {
            return
        }
...
# Defined in a separate module to separate the scopes so that
# the ScriptBlock won't inherit the vars from the function.
$null = New-Module {
    function Invoke-WithDevToolEnvironment {
        [CmdletBinding()]
        param (
            [Parameter()]
            [ScriptBlock]
        ...
        begin {
            ...
            $vsDevCmd = [Path]::Combine($installDir, "Common7", "Tools", "VsDevCmd.bat")
            $vsDevCmdArgs = @(
                "-arch=$($Architecture.ToString().ToLowerInvariant())"
                "-host_arch=$($env:PROCESSOR_ARCHITECTURE.ToLowerInvariant())"
                '&&'
                'set'
            )
            ...
            $start = $false
            cmd.exe /c $vsDevCmd @vsDevCmdArgs | ForEach-Object {
            ...
            foreach ($kvp in $devEnv.GetEnumerator()) {
        ...
        clean {
            foreach ($key in $devEnv.Keys) {
                if ($existingEnv.Contains($key)) {
                    Set-Item -LiteralPath "env:\$key" -Value $existingEnv[$key]
                }
                else {
